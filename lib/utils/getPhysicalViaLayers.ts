...
export function getPhysicalViaLayers({
  ...
  const boardLayers = layerCount === 1
    ? ["top"]
    : ["top", ...Array.from({ length: layerCount - 2 }, (_, i) => `inner${i + 1}`), "bottom"]
  const from = boardLayers.indexOf(fromLayer)
  const to = boardLayers.indexOf(toLayer)
  if (from < 0 || to < 0) {
    throw new Error(`Via transition ${fromLayer} -> ${toLayer} is outside the board layer stack`)
  }
  ...
  if (physicalLayers) {
    ...
    if (
      !occupied.has(fromLayer) || !occupied.has(toLayer) ||
      ordered.length !== physicalLayers.length || last - first + 1 !== ordered.length
    ) {
      throw new Error("Physical via layers must be a contiguous board span containing both transition layers")
    }
    return ordered
  }
