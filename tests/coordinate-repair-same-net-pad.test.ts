...
test("coordinate repair moves a via out of its own SMT pad while preserving terminals", (): void => {
  ...
  const srj: SimpleRouteJson = {
    ...
    obstacles: [{ type: "rect", center: { x: 0, y: 0 }, width: 0.6, height: 0.6,
      layers: ["top"], connectedTo: ["pcb_smtpad_same", "route"] }],
  }
  ...
  const engine = new AutoroutingDrcEngine(srj, { traceToPadClearance: 0.16, disallowViaInSmtPad: true })
