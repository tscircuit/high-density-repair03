...
test("cancels a redundant same-trace layer excursion between close vias", () => {
  const srj: SimpleRouteJson = {
    bounds: { minX: -2, minY: -2, maxX: 2, maxY: 2 },
    connections: [{ name: "source_net_5", pointsToConnect: [] }],
    obstacles: [],
    layerCount: 2,
    minTraceWidth: 0.1,
    minViaDiameter: 0.3,
  ...
  expect(changed).toBe(true)
  expect(routes[0]?.route.map(({ z }) => z)).toEqual([0, 0, 0, 0, 0, 0])
})

test("separates same-net vias when canonical topology repairs are blocked", () => {
  const srj: SimpleRouteJson = {
    bounds: { minX: -2, minY: -2, maxX: 2, maxY: 2 },
    connections: [{ name: "source_net_5", pointsToConnect: [] }],
    obstacles: [],
  ...
  expect(changed).toBe(true)
  expect(routes[0]!.route[1]!.x).toBeLessThan(0)
  expect(routes[0]!.route[4]!.x).toBeGreaterThan(0.05)
})
