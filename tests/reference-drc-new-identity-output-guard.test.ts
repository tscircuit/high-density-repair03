...
test("rejects an equal DRC count when the candidate creates a new error", () => {
  ...
  const referenceDrcEvaluator: DrcEvaluator = ({ routes }) => {
    ...
    if (Math.abs(viaX) <= 1e-6) {
      return [{ type: "pcb_trace_error", pcb_trace_error_id: "inherited" }]
    }
    return [{ type: "pcb_trace_error", pcb_trace_error_id: "new_error" }]
  }
  ...
  expect(solver.stats.globalDrcForceImproveReferenceInputDrcIssueCount).toBe(1)
  expect(
