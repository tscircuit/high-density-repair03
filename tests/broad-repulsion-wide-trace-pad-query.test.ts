...
test("broad pad repulsion uses the actual trace copper width across spatial buckets", () => {
  const srj: SimpleRouteJson = {
    layerCount: 2,
    minTraceWidth: 0.1,
    minTraceToPadEdgeClearance: 0.1,
    bounds: { minX: -3, maxX: 3, minY: -2, maxY: 5 },
    obstacles: [
      {
        type: "rect",
        center: { x: 0, y: 1.6 },
  ...
  expect(output!.traceThickness).toBe(1.2)
