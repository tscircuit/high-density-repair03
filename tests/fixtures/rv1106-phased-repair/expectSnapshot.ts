...
export const expectSnapshot = ({
  ...
  const svgSize = 640
  let svg = getSvgFromGraphicsObject(graphics, {
  ...
  if (relaxedDrcCount !== undefined) {
    ...
    svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${svgSize}" height="${svgSize + headerHeight}"><rect width="100%" height="100%" fill="white"/>${badge}<g transform="translate(0,${headerHeight})">${svg}</g></svg>`
  }
