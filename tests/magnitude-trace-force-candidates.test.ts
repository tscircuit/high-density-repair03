...
import { getDrcErrorForceCandidates, getDrcSnapshot } from "../lib/solvers/GlobalDrcForceImproveSolver/solverHelpers"
...
test("obstacle force candidates use distinct magnitudes only until full clearance displacement", () => {
  for (const { y, clearance, expectedCount } of [
    ...
    const candidates = [...getDrcErrorForceCandidates(
      srj, routes, before.errors[0]!, before.traceRouteIndexById, [1, 1.75, -1],
    )]
    expect(candidates).toHaveLength(expectedCount)
    expect(new Set(candidates.map((candidate) => JSON.stringify(candidate))).size).toBe(expectedCount)
    if (expectedCount === 1) {
      expect(getDrcSnapshot(srj, candidates[0]!, undefined, undefined, engine).count).toBe(0)
    }
